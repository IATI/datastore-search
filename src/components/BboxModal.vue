    ...
    >
        <div class="max-w p-6 bg-white divide-y divide-gray-500 rounded">
            ...
            <p v-if="global.state.bbox.southWestLat !== null">
                {{
                    global.state.bbox.southWestLat.toFixed(
                        global.state.bbox.displayPrecision,
                    )
                }},
                {{
                    global.state.bbox.southWestLon.toFixed(
                        global.state.bbox.displayPrecision,
                    )
                }}
                TO
                {{
                    global.state.bbox.northEastLat.toFixed(
                        global.state.bbox.displayPrecision,
                    )
                }},
                {{
                    global.state.bbox.northEastLon.toFixed(
                        global.state.bbox.displayPrecision,
                    )
                }}
            </p>
